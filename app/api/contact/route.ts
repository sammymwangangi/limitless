...
export async function POST(request: NextRequest) {
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0a1628;color:#e2e8f0;border-radius:12px;overflow:hidden;">
      <div style="background:#2563eb;padding:24px 32px;">
        <h1 style="margin:0;font-size:20px;color:#fff;">New Contact Form Submission</h1>
        <p style="margin:4px 0 0;color:#bfdbfe;font-size:14px;">Limitless Software Solutions</p>
      </div>
        ...
        <table style="width:100%;border-collapse:collapse;">
          ${[
            ...
            .map(
              ([label, value]) => `
            <tr>
              <td style="padding:8px 0;color:#94a3b8;font-size:13px;width:120px;">${label}</td>
              <td style="padding:8px 0;color:#e2e8f0;font-size:14px;font-weight:500;">${value}</td>
            </tr>`
            )
            .join("")}
        </table>
